...
This is the honest number for **threshold selection**: **Sharpe 0.249
out-of-sample**, against 0.264 for the untouched default and 0.316 for the
best in-sample config. The overfitting gap between "best in-sample" and
"walk-forward OOS" is real (0.316 to 0.249) but modest — smaller than a naive
reading of the heatmap would suggest, because the selected threshold turns
out to be fairly stable across folds (mostly `entry_z` 0.5–1.0, `exit_z`
0.25–0.5; see the stability figure) rather than whipsawing to a different
corner of the grid each year. Notably, the walk-forward curve tracks *below*
the untouched default for most of the sample and only pulls closer in
2025–2026 — selecting thresholds walk-forward is not free, but it is not
catastrophic either.

This number still prices every fold off one static, full-sample hedge ratio,
though — the next section checks whether that matters too.

## Does the hedge ratio itself need to be walk-forward too?

The walk-forward result above re-selects `entry_z`/`exit_z` each fold using
only prior data, but it still prices every fold off one static, full-sample
OLS beta from `src.models.spread` — fit using data through 2026-06-30,
including years any given early fold has not "seen" yet. That is a real, if
subtle, form of look-ahead: a trader live in 2017 would never have had
access to a hedge ratio informed by 2026 prices.

`src.strategy.walkforward_beta` closes this gap. For each fold, it refits
`fit_spread_model` (from `src.models.spread`) on rows strictly before that
fold's test period — the same expanding-window idea `subsample_stability`
already uses for a robustness check, applied here as the model itself —
builds that fold's own spread series from its own alpha/beta, and reruns the
identical grid-search-and-select machinery on it. Both the hedge ratio and
the thresholds are therefore chosen using only information that would have
existed at the time.

![Refit hedge ratio by fold](../outputs/figures/14_cl_bz_walkforward_beta_stability.png)

![Static-beta vs. beta-refit walk-forward equity](../outputs/figures/15_cl_bz_walkforward_beta_refit_equity.png)

| Metric | Static beta (above) | Beta refit per fold |
| --- | ---: | ---: |
| Sharpe net | 0.249 | **0.219** |
| Total net PnL | \$64,024 | \$55,954 |
| Max drawdown | −44.3% | −48.9% |
| Round-trip trades | 97 | 72 |

(Same sample: 2017-01-03 → 2026-06-30, n = 2,387. Full per-fold table:
`outputs/tables/backtest_walkforward_beta_refit.csv`; side-by-side summary:
`outputs/tables/backtest_summary_beta_refit.csv`.)

The refit beta stays in a tight 0.82–0.98 band across all ten folds (see
figure), consistent with the 0.74–1.14 full-sample rolling-beta range
`spread_model.md` already reported — CL/BZ's hedge ratio is genuinely
stable, not stable only because the static fit peeked at the future.
Removing the look-ahead costs about three points of Sharpe (0.249 → 0.219),
in the direction expected from estimating beta on less data each fold, not a
wide gap that would suggest the original static design was propped up by
information it should not have had.

**This is the final, fully leak-free headline number for the core strategy:
net Sharpe 0.22 out-of-sample, thresholds and hedge ratio both chosen
walk-forward.** Reproduce with `uv run python -m src.strategy.walkforward_beta`.
...
This module's own outputs are listed below; `src.strategy.walkforward_beta`
(see above) is what actually closes out the core pipeline — it extends this
stage's walk-forward validation to the hedge ratio itself and produces the
final, most-honest out-of-sample number. Everything below is gitignored
under `data/processed/`, regenerated by running `src.strategy.backtest`:
...
uv run python -m src.strategy.walkforward_beta  # ~90s: 10 folds, each refitting beta + rerunning the grid
uv run pytest -q                              # 85 tests, synthetic fixtures, no API key needed
```
