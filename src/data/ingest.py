"""Pull continuous daily futures series from Databento into data/raw and data/processed.
...
SCHEMA = "ohlcv-1d"  # daily bars are enough for pair EDA/backtesting and are free to pull
...
def fetch_continuous_daily(
    ...
    # boundaries can overlap by a day, so drop duplicate index entries.
...
def build_close_panel(raw_by_root: dict[str, pd.DataFrame]) -> pd.DataFrame:
    """Combine each root's raw OHLCV frame into one wide panel of close prices.
    ...
    """
    closes = {root: df["close"] for root, df in raw_by_root.items()}
...
def main(start: str = "2015-01-01", end: str = "2026-07-01") -> None:
    ...
    for root in ROOTS:
        ...
        # exact instrument_id/volume/OHLC fields Databento returns.
        df.to_parquet(RAW_DIR / f"{root}.c.0.parquet")
        raw_by_root[root] = df

    # Processed close-price panel: the single file downstream EDA/modeling
    # steps should read from, instead of re-assembling it from the raw files.
    panel = build_close_panel(raw_by_root)
    panel.to_parquet(PROCESSED_DIR / "continuous_close_prices.parquet")
    print(f"Saved {len(ROOTS)} raw series to {RAW_DIR} and close panel to {PROCESSED_DIR}")
